...
import {
  CvParsingService,
  ParsedCvData,
} from 'src/app/shared/servizi/cv-parsing.service';
import { CurrioService } from 'src/app/shared/servizi/currio.service';
import { Update } from '@ngrx/entity';
import * as CurrioActions from '../state/currio.action';
...
export class CurrioEditComponent implements OnInit, OnDestroy {
    ...
    if (this.router.url.startsWith('/admin/currio/edit/')) {
      this.route.paramMap.pipe(take(1)).subscribe((params) => {
        ...
        if (this.currioId) {
          ...
          this.handleCurrioLoadingError(
            'ID Curriò non fornito per la modifica admin.'
          );
        }
      });
      ...
      if (this.currentUserId) {
        ...
        const currioByUserSub = this.store
          .select(getCurrios)
          .pipe(
            map((currios) =>
              currios.find((c) => c.userId === this.currentUserId)
            ),
          ...
          .subscribe((userCurrio) => {
            if (userCurrio && userCurrio.id) {
              ...
              this.handleCurrioLoadingError(
                'Nessun Curriò trovato per questo account cliente.'
              );
            }
          });
        ...
        setTimeout(() => {
          if (this.isLoadingCurrio && !this.currentUserId) {
            this.handleCurrioLoadingError(
              "Impossibile recuperare l'ID utente. Prova a ricaricare la pagina."
            );
          }
        }, 5000);
      }
      ...
      this.handleCurrioLoadingError(
        'Rotta non riconosciuta o dati insufficienti per caricamento Curriò.'
      );
    }
  }

  private subscribeToSpecificCurrio(id: string): void {
    const currioSub = this.store
      .select(getCurrioById, { id })
      .pipe(
        filter(
          (currioLoaded): currioLoaded is Currio =>
            !!currioLoaded && !!currioLoaded.id
        ),
        take(1)
      )
      .subscribe(
        ...
        (error) => {
          this.handleCurrioLoadingError(
            'Errore durante il recupero dei dettagli del Curriò.',
            error
          );
        }
      );

    const timeoutSub = of(null)
      .pipe(
        take(1),
        tap(() => {
  ...
  initializeForm(): void {
    if (!this.currio) {
      this.handleCurrioLoadingError(
        'Tentativo di inizializzare il form senza dati Curriò.'
      );
      return;
    }
    this.currioForm.patchValue({
      ...
      contatti: {
        email:
          this.currio.contatti?.email || this.currio.datiCliente?.email || '',
    ...
    this.clearAndPopulateFormArray(
      this.softSkillsFormArray,
      this.currio.altreCompetenze?.softSkills,
      (skill: string) => this.fb.control(skill, Validators.required)
    );
    this.clearAndPopulateFormArray(
      this.lingueFormArray,
      this.currio.altreCompetenze?.lingue,
      this.createLinguaGroup.bind(this)
    );
    this.clearAndPopulateFormArray(
      this.progettiFormArray,
      this.currio.progetti,
      this.createProgettoGroup.bind(this)
    );
    this.clearAndPopulateFormArray(
      this.esperienzeFormArray,
      this.currio.esperienze,
      this.createEsperienzaGroup.bind(this)
    );
    this.clearAndPopulateFormArray(
      this.competenzeFormArray,
      this.currio.competenze,
      this.createCompetenzaGroup.bind(this)
    );
  ...
  private clearAndPopulateFormArray(
    formArray: FormArray,
    items: any[] | undefined,
    createGroupFn: (item?: any) => AbstractControl
  ): void {
  ...
  updateWarningState(): void {
    this.showUnsavedChangesWarning =
      this.currioForm && this.currioForm.dirty && !this.formError;
  }
  ...
  togglePrivacyStatus() {
    ...
    this.currio = { ...this.currio, status: newStatus };
  ...
  isAccordionSectionInvalid(
    section: 'progetti' | 'esperienze' | 'competenze' | 'altreCompetenze'
  ): boolean {
    const formGroup = this.currioForm.get(section);
    return (
      !!formGroup && formGroup.invalid && (formGroup.dirty || formGroup.touched)
    );
  }

  get softSkillsFormArray() {
    return this.currioForm.get('altreCompetenze.softSkills') as FormArray;
  }
  addSoftSkill(): void {
    this.softSkillsFormArray.push(this.fb.control('', Validators.required));
    this.currioForm.markAsDirty();
  }
  removeSoftSkill(index: number): void {
    this.softSkillsFormArray.removeAt(index);
    this.currioForm.markAsDirty();
  }

  get lingueFormArray() {
    return this.currioForm.get('altreCompetenze.lingue') as FormArray;
  }
  ...
  addLingua(): void {
    this.lingueFormArray.push(this.createLinguaGroup());
    this.currioForm.markAsDirty();
  }
  removeLingua(index: number): void {
    this.lingueFormArray.removeAt(index);
    this.currioForm.markAsDirty();
  }

  get progettiFormArray() {
    return this.currioForm.get('progetti') as FormArray;
  }
  createProgettoGroup(progetto?: CurrioProgetto): FormGroup {
    return this.fb.group({
      ...
      tags: this.fb.array(
        progetto?.tags?.map((tag) => this.fb.control(tag)) || []
      ),
    });
  }
  addProgetto(): void {
    this.progettiFormArray.push(this.createProgettoGroup());
    this.currioForm.markAsDirty();
  }
  removeProgetto(index: number): void {
    this.progettiFormArray.removeAt(index);
    this.currioForm.markAsDirty();
  }
  getTags(progettoIndex: number): FormArray {
    return this.progettiFormArray.at(progettoIndex).get('tags') as FormArray;
  }
  addTag(progettoIndex: number): void {
    this.getTags(progettoIndex).push(this.fb.control(''));
    this.currioForm.markAsDirty();
  }
  removeTag(progettoIndex: number, tagIndex: number): void {
    this.getTags(progettoIndex).removeAt(tagIndex);
    this.currioForm.markAsDirty();
  }

  get esperienzeFormArray() {
    return this.currioForm.get('esperienze') as FormArray;
  }
  createEsperienzaGroup(esperienza?: CurrioEsperienza): FormGroup {
    return this.fb.group({
      ...
      dettagli: this.fb.array(
        esperienza?.dettagli?.map((d) => this.fb.control(d)) || []
      ),
      competenzeAcquisite: this.fb.array(
        esperienza?.competenzeAcquisite?.map((c) => this.fb.control(c)) || []
      ),
    });
  }
  addEsperienza(): void {
    this.esperienzeFormArray.push(this.createEsperienzaGroup());
    this.currioForm.markAsDirty();
  }
  removeEsperienza(index: number): void {
    this.esperienzeFormArray.removeAt(index);
    this.currioForm.markAsDirty();
  }
  getDettagli(esperienzaIndex: number): FormArray {
    return this.esperienzeFormArray
      .at(esperienzaIndex)
      .get('dettagli') as FormArray;
  }
  addDettaglio(esperienzaIndex: number): void {
    this.getDettagli(esperienzaIndex).push(this.fb.control(''));
    this.currioForm.markAsDirty();
  }
  removeDettaglio(esperienzaIndex: number, dettaglioIndex: number): void {
    this.getDettagli(esperienzaIndex).removeAt(dettaglioIndex);
    this.currioForm.markAsDirty();
  }
  getCompetenzeAcquisite(esperienzaIndex: number): FormArray {
    return this.esperienzeFormArray
      .at(esperienzaIndex)
      .get('competenzeAcquisite') as FormArray;
  }
  addCompetenzaAcquisita(esperienzaIndex: number): void {
    this.getCompetenzeAcquisite(esperienzaIndex).push(this.fb.control(''));
    this.currioForm.markAsDirty();
  }
  removeCompetenzaAcquisita(esperienzaIndex: number, compIndex: number): void {
    this.getCompetenzeAcquisite(esperienzaIndex).removeAt(compIndex);
    this.currioForm.markAsDirty();
  }

  get competenzeFormArray() {
    return this.currioForm.get('competenze') as FormArray;
  }
  ...
  addCompetenza(): void {
    this.competenzeFormArray.push(this.createCompetenzaGroup());
    this.currioForm.markAsDirty();
  }
  removeCompetenza(index: number): void {
    this.competenzeFormArray.removeAt(index);
    this.currioForm.markAsDirty();
  }
  ...
  preparaEInviaInvito(): void {
    if (
      ...
      !this.currio ||
      !this.currio.id ||
      !this.currio.datiCliente?.email
    ) {
      Swal.fire(
        'Errore',
        'Operazione non permessa o dati del curriò/cliente mancanti.',
        'error'
      );
      return;
    }
  ...
  onSubmit(): void {
    if (!this.currioForm.valid) {
      ...
      this.formError =
        'Attenzione, alcuni campi non sono validi. Controlla le sezioni evidenziate e correggi gli errori.';
    ...
    if (!this.currio || !this.currio.id) {
      Swal.fire(
        'Errore',
        'Impossibile salvare, dati Curriò di riferimento mancanti o ID non valido.',
        'error'
      );
      return;
    }
    ...
    const currioToUpdate: Currio = {
      ...
      progetti: formValue.progetti.map((p: CurrioProgetto) => ({
        ...p,
        id: p.id || uuidv4(),
      })),
      esperienze: formValue.esperienze.map((e: CurrioEsperienza) => ({
        ...e,
        id: e.id || uuidv4(),
      })),
      competenze: formValue.competenze.map((c: CurrioCompetenza) => ({
        ...c,
        id: c.id || uuidv4(),
      })),
  ...
  openCurriculum(): void {
    if (this.isUserAdmin && this.currio && this.currio.curriculumUrl) {
      ...
      Swal.fire(
        'Attenzione',
        'Nessun CV allegato o operazione non permessa.',
        'warning'
      );
    }
  }
  ...
  onDeleteCurrio(): void {
    if (this.isUserAdmin && this.currio && this.currio.id) {
      ...
      Swal.fire(
        'Errore',
        'Operazione non permessa o ID del Curriò non disponibile.',
        'error'
      );
    }
  }
  ...
  autoFillFromCv(): void {
    if (!this.currio?.curriculumUrl) {
      Swal.fire(
        'Attenzione',
        'Nessun curriculum caricato per questo Curriò.',
        'warning'
      );
      return;
    }
    ...
    this.cvParsingService.parseCvFromUrl(this.currio.curriculumUrl).subscribe({
      next: (parsedData) => {
        this.clearAndPopulateFormArray(
          this.esperienzeFormArray,
          parsedData.esperienze,
          this.createEsperienzaGroup.bind(this)
        );
        this.clearAndPopulateFormArray(
          this.competenzeFormArray,
          parsedData.competenze,
          this.createCompetenzaGroup.bind(this)
        );
        this.clearAndPopulateFormArray(
          this.progettiFormArray,
        ...
        if (parsedData.altreCompetenze) {
          this.clearAndPopulateFormArray(
            this.softSkillsFormArray,
            parsedData.altreCompetenze.softSkills,
            (skill: string) => this.fb.control(skill, Validators.required)
          );
          this.clearAndPopulateFormArray(
            this.lingueFormArray,
            parsedData.altreCompetenze.lingue,
            this.createLinguaGroup.bind(this)
          );
        }
        ...
        Swal.fire(
          'Successo!',
          'I campi sono stati compilati con le informazioni estratte dal CV.',
          'success'
        );
      },
      error: (err) => {
        ...
        Swal.fire(
          'Errore',
          err.message || "Si è verificato un errore durante l'analisi del CV.",
          'error'
        );
      },
    });
      ...
      if (result.isConfirmed) {
        ...
        this.currioService
          ...
          .catch((error) => {
            ...
            Swal.fire(
              'Errore',
              'Impossibile eliminare il file del CV.',
              'error'
            );
            this.store.dispatch(setLoadingSpinner({ status: false }));
          });
